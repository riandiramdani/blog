...
export default {
  ...
  async asyncData({store, params, app, error}) {
    try {
      ...
      let article = await app.$api.getArticlesByCategory(categoryId)
      let popularArticles = await app.$api.getPopularArticlesByCategory(categoryId)
  ...
  methods: {
    ...
    loadMore() {
      ...
      this.$api.getArticlesByCategory(this.categoryId, this.meta.current_page + 1)
        .then(res => {
          let data = res.data
          this.meta = data.meta
